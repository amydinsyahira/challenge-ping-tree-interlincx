    ...
    try {
      ...
      redis.hget('accept_geostate', geoState, function (err, doc) {
        ...
        redis.hget('accept_hour', hourParam.getUTCHours().toString(), function (err, doc) {
          ...
          if (!filtered) return res.writeHead(406, { 'Content-Type': 'application/json' }).end(JSON.stringify({ decision: 'reject' }))

          let highestValue = 0, url = ''
            ...
            redis.hget('targets', id, function (err, doc) {
              ...
              redis.get(`accept_count:${selectId}`, function (e, doc) {
                ...
                if (!doc) {
                  ...
                  redis.expireat(`accept_count:${selectId}`, midnight.getTime())
                  return res.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify({ url }))
                }
