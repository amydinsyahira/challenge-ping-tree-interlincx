    ...
    try {
      ...
      redis.hlen('targets', function (e, count) {
        ...
        redis.hset('targets', newId, JSON.stringify({
          ...
          res.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify({ message: 'The target has been created' }))
        })
      })
    } catch (e) {
...
function onIndexGet(req, res) {
  try {
    ...
    redis.hgetall('targets', function (e, doc) {
      if (e) return res.writeHead(500).end(e.message)
      if (!doc) return res.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify({ message: 'The target data was empty' }))
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify({ data: targets, count }))
    })
  } catch (e) {
    console.error(e.message)
    res.writeHead(500).end(e.message)
  ...
  req.on('end', function () {
    try {
      ...
      redis.hget('targets', id, function (e, doc) {
        if (e) return res.writeHead(500).end(e.message)
        if (!doc) return res.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify({ message: 'The target data was not found' }))

        doc = JSON.parse(doc)
        
        let oldDoc = doc
        let newDoc = { id, url: url || oldDoc.url, value: value || oldDoc.value, maxAcceptsPerDay: maxAcceptsPerDay || oldDoc.maxAcceptsPerDay, accept: { geoState: { "$in": acceptGeoState || oldDoc.accept.geoState["$in"] }, hour: { "$in": acceptHour || oldDoc.accept.hour["$in"] } } }
        ...
        redis.hset('targets', id, JSON.stringify(newDoc), function (e) {
          ...
          res.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify({ message: 'The target has been updated' }))
        })
      })
    } catch (e) {
      if (e.message.indexOf('json')) return res.writeHead(400).end('400 Bad Request')
...
function onIdGet(req, res, opts) {
  try {
    ...
    redis.hget('targets', id, function (e, doc) {
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify({ data: JSON.parse(doc) }))
    })
  } catch (e) {
    console.error(e.message)
    res.writeHead(500).end(e.message)
